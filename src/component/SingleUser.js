...
const SingleUser = ({ u, user, email, role, plan, status, action }) => {
  return (
    <tr className=" border-[2px]">
      ...
      {!role && (
        <td className=" px-4 py-2 text-gray-500 flex items-center">
          <span className=" text-yellow-500 text-xl">
            {u.role === "Admin" && <BsLaptop></BsLaptop>}
          </span>
          <span className="mr-2 text-xl">
            {u.role === "Editor" && <MdOutlineModeEdit></MdOutlineModeEdit>}
          </span>
          <span className="mr-2 text-xl">
            {u.role === "Author" && <AiFillSetting></AiFillSetting>}
          </span>
          <span className="text-yellow-500 text-2xl">
            {u.role === "Maintianer" && <CgShapeCircle></CgShapeCircle>}
          </span>
          <span className="mr-2 text-xl">
            {u.role === "Subscriber" && <AiOutlineUser></AiOutlineUser>}
          </span>
      ...
      {!action && (
        <td className=" px-4 py-5 flex justify-end items-center  border-none text-xl">
